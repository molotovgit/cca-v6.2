...
async function rotateChatgpt() {
  ...
  const rot = runShell(PYTHON, ['-m', 'src.python.auth.accounts', 'rotate', 'chatgpt', '--wrap']);
  if (rot.code !== 0) {
  ...
  const login = runShell(PYTHON, ['src/python/auth/auto_login.py', '--skip-gemini', '--force-resignin'], { stdio: 'inherit' });
  if (login.code !== 0) {
...
function findChapterFile(stage, ext = '.md') {
  ...
  const folder = path.join(REPO, 'data', stage, `g${CONFIG.GRADE}-${CONFIG.LANG}`, subjectSlug);
...
async function stageFetch() {
  ...
  await runChild(PYTHON, [
    'src/python/pipeline/fetch_chapter.py',
...
async function stageRefine() {
  ...
  await runChatgptStage('REFINE', 'src/python/pipeline/refine_chapter.py', [
    '--grade',   String(CONFIG.GRADE),
    '--lang',    CONFIG.LANG,
    '--subject', CONFIG.SUBJECT,
    '--chapter', String(CONFIG.CHAPTER),
...
async function stagePrompts() {
  ...
  await runChatgptStage('PROMPTS', 'src/python/pipeline/generate_prompts.py', [
    '--grade',   String(CONFIG.GRADE),
    '--lang',    CONFIG.LANG,
    '--subject', CONFIG.SUBJECT,
    '--chapter', String(CONFIG.CHAPTER),
...
async function stageImages(promptsJson) {
  ...
  const imagesDir = path.join(REPO, 'data', 'images', `g${CONFIG.GRADE}-${CONFIG.LANG}`, subjectSlug, base);
  ...
  await new Promise((resolve, reject) => {
    ...
    const args = isWin
      ? ['/c', 'node', 'src\\node\\orchestrators\\run_autonomous.cjs', promptsJson]
      : ['src/node/orchestrators/run_autonomous.cjs', promptsJson];
    const child = spawn(cmd, args, {
      cwd: REPO,
      stdio: 'inherit',
      env: process.env,
...
async function stageUpload() {
  ...
  await runChild(PYTHON, [
    'src/python/pipeline/upload_images.py',
    '--grade',   String(CONFIG.GRADE),
    '--lang',    CONFIG.LANG,
    '--subject', CONFIG.SUBJECT,
    '--chapter', String(CONFIG.CHAPTER),
...
(async () => {
  ...
  if (!chatgpt) { console.error('\n  ✗ ChatGPT Chrome not reachable. Start: python src/python/infra/chrome_keepalive.py\n'); process.exit(2); }
  ...
  const subjectSlug = slugify(CONFIG.SUBJECT);
  const base = path.basename(promptsJson || '', '.json');
  console.log(`  Chapter:  data/chapters/g${CONFIG.GRADE}-${CONFIG.LANG}/${subjectSlug}/`);
  console.log(`  Refined:  data/refined/g${CONFIG.GRADE}-${CONFIG.LANG}/${subjectSlug}/`);
  console.log(`  Prompts:  data/prompts/g${CONFIG.GRADE}-${CONFIG.LANG}/${subjectSlug}/${base}.json`);
  console.log(`  Images:   data/images/g${CONFIG.GRADE}-${CONFIG.LANG}/${subjectSlug}/${base}/  (80 PNGs)`);
  console.log(`  Zip:      data/zips/g${CONFIG.GRADE}-${CONFIG.LANG}/${subjectSlug}/${base}.zip  (uploaded to Notion)`);
  console.log('');
})();
