...
function findLatestLog() {
  const reports = path.join(REPO, 'data', 'reports');
...
function enumerateAllLessons() {
  ...
  const out = [];
  const chaptersRoot = path.join(REPO, 'data', 'chapters');
  ...
  for (const gradeDir of gradeDirs) {
    ...
    for (const subjSlug of subjDirs) {
      ...
      for (const metaFile of metaFiles) {
        ...
        // Stage 2: refined MD
        const refinedMd = path.join(REPO, 'data', 'refined', gradeDir, subjSlug, `${base}.md`);
        const refineDone = fileExists(refinedMd);

        // Stage 3: prompts JSON (count entries)
        const promptsJson = path.join(REPO, 'data', 'prompts', gradeDir, subjSlug, `${base}.json`);
        ...
        const imagesDir = path.join(REPO, 'data', 'images', gradeDir, subjSlug, base);
        const imagesCount = countFiles(imagesDir, '.png');

        // Stage 5: upload marker
        const markerPath = path.join(REPO, 'data', 'zips', gradeDir, subjSlug, `${base}.uploaded.json`);
        const marker = fileExists(markerPath) ? safeJSON(markerPath, null) : null;
        const zipPath = path.join(REPO, 'data', 'zips', gradeDir, subjSlug, `${base}.zip`);
...
function snapshot() {
  ...
  if (out.state && out.state.currentLesson) {
    ...
    const promptsDir = path.join(REPO, 'data', 'prompts', `g${grade}-${lang}`, subjectSlug);
    ...
    if (promptsBase) {
      ...
      const imagesDir = path.join(REPO, 'data', 'images', `g${grade}-${lang}`, subjectSlug, promptsBase);
      const imageCount = countFiles(imagesDir, '.png');
      const zipPath = path.join(REPO, 'data', 'zips', `g${grade}-${lang}`, subjectSlug, `${promptsBase}.zip`);
      const uploadedMarker = path.join(REPO, 'data', 'zips', `g${grade}-${lang}`, subjectSlug, `${promptsBase}.uploaded.json`);
      out.diskCounts = {
  ...
  // .cca state files
  out.cca = {
    saved_indices: safeJSON(path.join(REPO, 'data', '.cca', 'saved_indices.json'), null),
  ...
  const activeAccounts  = safeJSON(path.join(REPO, 'data', '.cca', 'active_accounts.json'), {}) || {};
