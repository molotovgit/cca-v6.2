...
If Chrome is in a non-standard location, the script may not find it. Edit the `CHROME_PATHS` array in `src/node/setup/setup_chrome.cjs` to point at your `chrome.exe`.
...
The `src/python/pipeline/fetch_chapter.py` script expects this exact hierarchy in your Notion workspace:
...
- Edit `src/python/drivers/notion/navigator.py` to match your structure
...
All outputs land under the `data/` folder:

```
data/chapters/g{GRADE}-{LANG}/{subject-slug}/ch{NN}-{title}.md     ← Notion fetch
data/refined/g{GRADE}-{LANG}/{subject-slug}/ch{NN}-{title}.md       ← ChatGPT-refined
data/prompts/g{GRADE}-{LANG}/{subject-slug}/ch{NN}-{title}.json    ← 80 prompts
data/images/g{GRADE}-{LANG}/{subject-slug}/ch{NN}-{title}/*.png    ← 80 PNGs
data/zips/g{GRADE}-{LANG}/{subject-slug}/ch{NN}-{title}.zip        ← uploaded to Notion
data/zips/g{GRADE}-{LANG}/{subject-slug}/ch{NN}-{title}.uploaded.json   ← upload marker (sha256 + block_id)
```
...
| `config/examples/lessons.txt.example` | Template (committed; copied to `lessons.txt` by `start.bat` on first run) |
...
| `config/prompts/80_prompt_formula.txt` | The formula ChatGPT uses to generate image prompts |
| `docs/user/GUIDE.md` | Quick-reference guide for day-to-day use |
