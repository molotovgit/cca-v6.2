...
1. **FETCH** — pulls textbook chapter from Notion (`src/python/pipeline/fetch_chapter.py`)
2. **REFINE** — ChatGPT rewrites it (`src/python/pipeline/refine_chapter.py`, ChatGPT on Chrome :9222)
3. **PROMPTS** — ChatGPT generates 80 image prompts (`src/python/pipeline/generate_prompts.py`)
4. **IMAGES** — Gemini renders 80 PNGs in 10 parallel tabs (`src/node/orchestrators/run_autonomous.cjs`, Gemini on Chrome :9223)
5. **UPLOAD** — `src/python/pipeline/upload_images.py` zips refined MD + 80 PNGs, attaches the zip as a file block on the chapter's "Images" subpage in Notion

Each stage is idempotent — skips if its output is already on disk. Per-chapter driver: `src/node/orchestrators/run_pipeline.cjs`. Multi-chapter driver: a `.bat` loop (see "Launcher pattern" below). **Do not use `src/node/orchestrators/run_batch.cjs`** — it has a broken Windows workaround that races chapters in parallel.

## Critical files

| File | Purpose |
|---|---|
| `src/node/orchestrators/run_pipeline.cjs` | Per-chapter 5-stage orchestrator. Reads `CCA_GRADE`/`CCA_LANG`/`CCA_SUBJECT`/`CCA_CHAPTER` env vars. |
| `src/node/orchestrators/run_autonomous.cjs` | IMAGES-stage orchestrator. Handles rotation, rescue, saver-restart. |
| `src/node/workers/save_images.cjs` | Pulls PNGs off Gemini tabs. Has `detectBlocker()` for explicit 1095/quota UI. |
...
2. `python -m src.python.auth.accounts reset gemini` so each batch run starts on `primary` (older runs may have left state on a backup)
...
5. `node src\node\setup\setup_chrome.cjs` to launch CDP Chrome on :9222 and :9223 (skips if already up)
6. `start "CCA Dashboard" cmd /k "node src\node\dashboard\dashboard.cjs"` then `start "" "http://localhost:7777"` — `cmd /k` (not `/MIN`) so the user can see errors if the dashboard crashes
7. `python src/python/auth/auto_login.py` — pause for manual sign-in if rc != 0 (CAPTCHA on first-time profile is normal)
8. Per-chapter blocks: `set CCA_CHAPTER=N` then `node src\node\orchestrators\run_pipeline.cjs <nul`. **The `<nul` redirect is required** — without it the child's stdio handle can disrupt the parent batch script's stdin and the .bat exits prematurely (cmd's invisible "Terminate batch job?" auto-Y).
...
- **Never automate Google sign-in past the first manual confirmation.** First-time use of a new account on a fresh Chrome profile triggers reCAPTCHA / "verify it's you" — `src/python/auth/auto_login.py` returns rc=1 in that case and `launch_*.bat` pauses for manual sign-in. Do not try to bypass.
- **Do not reintroduce `src/node/orchestrators/run_batch.cjs`** as the multi-chapter driver. Its `spawn(detached: true)` Windows workaround breaks the parent's `await` and races chapters in parallel.
- **Do not remove the `<nul` redirect** from any `node src\node\orchestrators\run_pipeline.cjs` call in a .bat file.
- **`pip install`** must always be `python -m pip install` (pip is often not on PATH).
- **`src/python/pipeline/refine_chapter.py` and `src/node/orchestrators/run_pipeline.cjs` preflight** still hard-require `CHATGPT_EMAIL` / `CHATGPT_PASSWORD` in `.env` even though v6's documented credential source is `accounts.json`. The launcher mirrors the primary creds into `.env`. Don't "fix" the env requirement without also patching those two callsites. (Line numbers shift across refactors — search for the env var names.)

## Rotation flow (read this before touching `src/python/auth/auto_login.py`, `src/python/drivers/browser/gemini.py`, or `src/node/orchestrators/run_autonomous.cjs`)

Triggered by either:
- Explicit alert: `src/node/workers/save_images.cjs::detectBlocker()` writes to `.cca/blocker_alerts.json`, orchestrator's `shouldRotate()` reads it
...
### UPLOAD stage (`src/python/pipeline/upload_images.py`)
...
- `src/node/orchestrators/run_batch.cjs:144` — `detached: process.platform === 'win32'` workaround. Replaced by shell-level loops in `launch_*.bat`. Don't re-enable as the multi-chapter driver.
- `src/python/pipeline/refine_chapter.py`, `src/python/pipeline/generate_prompts.py`, `src/node/orchestrators/run_pipeline.cjs` preflight — hard-require `CHATGPT_EMAIL` / `CHATGPT_PASSWORD` in `.env` despite `accounts.json` being v6's documented source. Launcher mirrors the primary creds into both. (Line numbers shift across refactors — search for the env var names.)
